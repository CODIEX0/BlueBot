...
export function MobileDatabaseProvider({ children }: MobileDatabaseProviderProps) {
  ...
  const addExpense = async (expense: Omit<Expense, 'id' | 'userId' | 'createdAt' | 'updatedAt' | 'syncStatus'>) => {
    ...
    try {
      ...
      `, [
        currentUser.id, expense.amount, expense.category, expense.merchant,
      ...
      // Queue for sync
      const newExpense = { ...expense, id: result.lastInsertRowId, userId: currentUser.id, createdAt: now, updatedAt: now };
